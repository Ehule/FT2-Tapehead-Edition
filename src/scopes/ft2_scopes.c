	...
	uint16_t scopeLen, int32_t channelIndex)
{
	...
	/* Keep the output marker clear of the configurable trim strip. */
	const uint8_t trimWidth = tapeheadConfig.trackTrimDisplayWidth;
	int16_t x = scopeXOffs + scopeLen - 6 - (trimWidth == 0 ? 0 : trimWidth + 2);
	if (alsoToMain)
	{
		x -= 7;
...
static void drawTrackTrimIndicator(uint16_t scopeX, uint16_t scopeY,
	uint16_t scopeLen, int32_t channelIndex)
{
	const uint8_t width = tapeheadConfig.trackTrimDisplayWidth;
	if (width == 0)
		return;
	...
	hLine(x - 1, unityY, width + 2, PAL_FORGRND);
}
