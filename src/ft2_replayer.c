...
/*
** Runtime-only mixer-style mute.
**
** Unlike FT2's normal channel mute, this does not stop the voice or prevent
** pattern data/effects from being processed. It only silences mixer output.
*/
...
#ifdef HAS_MIDI
static uint8_t midiDubVelocityFromNoteVolume(const channel_t *ch, uint8_t efx, uint8_t efxData)
{
	uint8_t volume = ch->oldVol; // sample default volume (0..64)

	// A direct volume-column value takes effect on the note's first tick.
	if (ch->volColumnVol >= 0x10 && ch->volColumnVol <= 0x50)
		volume = ch->volColumnVol - 0x10;

	// Cxx also sets the note volume on tick zero.
	if (efx == 0x0C)
		volume = efxData > 64 ? 64 : efxData;

	// MIDI velocity zero means Note Off, so map FT2 volume 0..64 to 1..127.
	return (uint8_t)(1 + ((volume * 126u + 32u) / 64u));
}
#endif
...
static void tapeheadEffects_TickZero(channel_t *ch, uint8_t param)
{
	...
	** configure latent state even while the global Fast Tracks logo is off. */
	...
	/* Z12/Z13 explicitly disengage/engage FastTracks for this track,
	** matching the Ctrl+Shift track toggle without relying on toggle state.
	** Z14 performs a one-shot sync while preserving the current ratio. */
	if (param == 0x12)
	{
		fastTracksPOCSetTrackEnabled(channelIndex, false);
		return;
	}
	else if (param == 0x13)
	{
		fastTracksPOCSetTrackEnabled(channelIndex, true);
		return;
	}
	else if (param == 0x14)
	{
		fastTracksPOCSyncTrackToMaster(channelIndex);
		return;
	}

	/* Z15/Z16 explicitly select the per-track private transport. Entering
	** either mode from Standard initializes that transport at the current
	** master position; switching Pattern/Song preserves the established
	** private displacement according to fastTracksPOCSetMode().
	**
	** Z17 returns only this track to 1:1 while preserving its private phase.
	** Z18 performs the same normalization and then hard-syncs the track. */
	if (param == 0x15)
	{
	...
	{
		fastTracksPOCSetRatioIndex(channelIndex, FAST_TRACKS_ONE_TO_ONE_RATIO_INDEX);
		return;
	}
	else if (param == 0x18)
	{
...
void updateVolPanAutoVib(channel_t *ch)
{
	...
	if (!ch->mute)
	{
		...
		// Apply non-destructive per-channel output trim.
		const int32_t channelIndex = (int32_t)(ch - channel);
	...
	for (int32_t i = 0; i < song.numChannels; i++, ch++)
	{
		...
		if (fastTrackEnabled && !fastTracksPOCIsClutched(i) && song.pattDelTime2 == 0)
		{
			...
			** this same per-tick rational transport. */
			...
			if (crossingCount > 0)
			{
				/* Process every crossed row in transport order. Several crossings still
				** share one mixer tick, but intermediate note-offs, instruments, volume
				** and effects are no longer silently discarded. */
			...
			/* At master tick zero without a private row advance, hold the
			** current Fast Track voice exactly as the slower-ratio prototype did. */
		}
		else if (readNewNote)
		{
