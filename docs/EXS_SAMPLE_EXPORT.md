...
2. Edit or batch-process the WAV files in TapeSister, REAPER, or another audio tool. Preserve their relative paths and do not rename or replace the manifest.
...
6. Inspect the export source, current module, the first 12 exact destination mappings, original/new frame lengths, the remaining mapping count, and any loop warnings. A source-name mismatch is called out explicitly. Confirm only when those destinations are correct; the manifest contains the complete list for a larger export.
7. Save the result as a new XM. One **Undo** reverses the complete accepted replacement.

Tapehead requires every manifest WAV to exist and decode successfully before it offers the confirmation. It then replaces only the listed `InstrumentIndex`/`SampleIndex` slots; unlisted instruments, unlisted sample slots, note maps, envelopes, and other instrument settings remain intact. Cancellation, an invalid path, malformed metadata, missing/invalid WAV, allocation failure, or an import larger than the configured Undo memory limit changes nothing.
