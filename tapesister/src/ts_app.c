...
bool ts_app_select_recipe(ts_app_state*a,size_t index){if(!a||index>=a->bank_count)return false;if(index==a->selected)return true;ts_rendered_sample candidate={0};ts_render_report report;if(!ts_render(&a->bank[index].recipe,&candidate,&report))return false;ts_rendered_sample_free(&candidate);uint64_t generation=ts_render_worker_request(a->render_worker,&a->bank[index].recipe);if(!generation)return false;a->selected=index;a->session_identity++;a->working_generation=generation;a->failed_generation=0;a->render_error[0]=0;ts_recipe_history_destroy(&a->history);ts_owned_recipe_destroy(&a->parent);a->has_parent=false;a->has_baked=false;ts_owned_recipe_destroy(&a->saved);a->has_saved=false;a->saved_path[0]=0;return true;}
