...
int main(int argc, char **argv) {
  ...
  while (running) {
    ...
    while (SDL_PollEvent(&e)) {
      if (e.type == SDL_QUIT)
      ...
      } else if(e.type==SDL_TEXTINPUT&&modal!=MODAL_NONE){overwrite_confirm=false;discard_confirm=false;ts_text_edit_insert(browser_open?&browser.filename:&modal_edit,e.text.text);}
        ...
        if(browser_open&&e.type==SDL_MOUSEWHEEL){if(e.wheel.y>0&&browser.selected>0)browser.selected--;else if(e.wheel.y<0&&browser.selected+1<browser.count)browser.selected++;}
        else if(browser_open&&e.type==SDL_MOUSEBUTTONDOWN&&e.button.button==SDL_BUTTON_LEFT){int lx,ly;if(mouse_to_logical(window,renderer,&presentation,e.button.x,e.button.y,&lx,&ly)){if(ly>=101&&ly<231&&lx>=82&&lx<538){size_t row=(size_t)((ly-101)/13)+browser.scroll;if(row<browser.count){browser.selected=row;if(e.button.clicks>=2)ts_file_browser_enter(&browser,row);}}else if(ly>=260&&ly<286){if(lx<180)ts_file_browser_home(&browser);else if(lx<280)ts_file_browser_root(&browser);else if(lx<390)ts_file_browser_parent(&browser);else ts_file_browser_mkdir(&browser,browser.filename.text);}}}
      }
