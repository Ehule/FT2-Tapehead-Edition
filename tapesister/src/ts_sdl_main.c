  ...
  while (running) {
    ...
    while (SDL_PollEvent(&e)) {
      if (e.type == SDL_QUIT)
        ...
        if(browser_open&&e.type==SDL_MOUSEWHEEL)ts_file_browser_move(&browser,e.wheel.y>0?TS_BROWSER_KEY_UP:TS_BROWSER_KEY_DOWN);
